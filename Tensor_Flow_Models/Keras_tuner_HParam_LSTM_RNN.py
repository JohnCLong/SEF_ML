...
def RNN_model(hp):
    ...
    for i in range(hp.Range('num_layers', 0, 10)):
        model.add(keras.layers.LSTM(units=hp.Range('units_' + str(i), min_value=1, max_value=501, step=10),
                                         input_shape=[None, 1],
                                         activation="relu",
                                         return_sequences=True))
        model.add(keras.layers.Dropout(hp.Choice('Drop_out', [0, 0.01, 0.1, 0.4, 0.6], default=0)))
    model.add(keras.layers.LSTM(units=hp.Range('units_last', min_value=1, max_value=501, step=10),
                                     input_shape=[None, 1],
                                     activation="relu"))
...
tuner = RandomSearch(
    ...
    directory='Tensor_Flow_Models/LSTM_RNN_test/keras_tuner',
...
plt.plot(range(729),y_train, "b")
plt.plot(range(729),y_pred, "--")
plt.show()
